...
class Roboclaw:
        ...
        if val1[0]:
            ...
            if val2[0]:
                ...
                if val3[0]:
                    val4 = self._readbyte()
                    if val4[0]:
                        return (1, val1[1] << 24 | val2[1] << 16 | val3[1] << 8
                                | val4[1])
        return (0, 0)
    ...
    def _write4S444S441(self, address, cmd, val1, val2, val3, val4, val5, val6,
                        val7):
        trys = self._trystimeout
        while trys:
    ...
    def _write4444444(self, address, cmd, val1, val2, val3, val4, val5, val6,
                      val7):
        trys = self._trystimeout
        while trys:
            self._sendcommand(address, cmd)
    ...
    def _write444444441(self, address, cmd, val1, val2, val3, val4, val5, val6,
                        val7, val8, val9):
        trys = self._trystimeout
        while trys:
            self._sendcommand(address, cmd)
...
#					str+=data[0]
    ...
    def SetM1VelocityPID(self, address, p, i, d, qpps):
        ...
        return self._write4444(address, self.Cmd.SETM1PID, d * 65536,
                               p * 65536, i * 65536, qpps)
    ...
    def SetM2VelocityPID(self, address, p, i, d, qpps):
        ...
        return self._write4444(address, self.Cmd.SETM2PID, d * 65536,
                               p * 65536, i * 65536, qpps)
    ...
    def SpeedAccelM1M2(self, address, accel, speed1, speed2):
        return self._write4S4S4(address, self.Cmd.MIXEDSPEEDACCEL, accel,
                                speed1, speed2)

    def SpeedDistanceM1(self, address, speed, distance, buffer):
        return self._writeS441(address, self.Cmd.M1SPEEDDIST, speed, distance,
                               buffer)

    def SpeedDistanceM2(self, address, speed, distance, buffer):
        return self._writeS441(address, self.Cmd.M2SPEEDDIST, speed, distance,
                               buffer)

    def SpeedDistanceM1M2(self, address, speed1, distance1, speed2, distance2,
                          buffer):
        return self._writeS44S441(address, self.Cmd.MIXEDSPEEDDIST, speed1,
                                  distance1, speed2, distance2, buffer)

    def SpeedAccelDistanceM1(self, address, accel, speed, distance, buffer):
        return self._write4S441(address, self.Cmd.M1SPEEDACCELDIST, accel,
    ...
    def SpeedAccelDistanceM1M2(self, address, accel, speed1, distance1, speed2,
                               distance2, buffer):
        return self._write4S44S441(address, self.Cmd.MIXEDSPEEDACCELDIST,
                                   accel, speed1, distance1, speed2, distance2,
                                   buffer)
    ...
    def SpeedAccelM1M2_2(self, address, accel1, speed1, accel2, speed2):
        return self._write4S44S4(address, self.Cmd.MIXEDSPEED2ACCEL, accel,
                                 speed1, accel2, speed2)

    def SpeedAccelDistanceM1M2_2(self, address, accel1, speed1, distance1,
                                 accel2, speed2, distance2, buffer):
        return self._write4S444S441(address, self.Cmd.MIXEDSPEED2ACCELDIST,
                                    accel1, speed1, distance1, accel2, speed2,
                                    distance2, buffer)
    ...
    def DutyAccelM1M2(self, address, accel1, duty1, accel2, duty2):
        return self._writeS24S24(address, self.Cmd.MIXEDDUTYACCEL, duty1,
                                 accel1, duty2, accel2)
    ...
    def SetM1PositionPID(self, address, kp, ki, kd, kimax, deadzone, min, max):
        ...
        return self._write4444444(address, self.Cmd.SETM1POSPID, kd * 1024,
                                  kp * 1024, ki * 1024, kimax, deadzone, min,
                                  max)
    ...
    def SetM2PositionPID(self, address, kp, ki, kd, kimax, deadzone, min, max):
        ...
        return self._write4444444(address, self.Cmd.SETM2POSPID, kd * 1024,
                                  kp * 1024, ki * 1024, kimax, deadzone, min,
                                  max)
    ...
    def SpeedAccelDeccelPositionM1(self, address, accel, speed, deccel,
                                   position, buffer):
        return self._write44441(address, self.Cmd.M1SPEEDACCELDECCELPOS, accel,
                                speed, deccel, position, buffer)

    def SpeedAccelDeccelPositionM2(self, address, accel, speed, deccel,
                                   position, buffer):
        return self._write44441(address, self.Cmd.M2SPEEDACCELDECCELPOS, accel,
                                speed, deccel, position, buffer)

    def SpeedAccelDeccelPositionM1M2(self, address, accel1, speed1, deccel1,
                                     position1, accel2, speed2, deccel2,
                                     position2, buffer):
        return self._write444444441(address, self.Cmd.MIXEDSPEEDACCELDECCELPOS,
                                    accel1, speed1, deccel1, position1, accel2,
                                    speed2, deccel2, position2, buffer)
    ...
    def SetPinFunctions(self, address, S3mode, S4mode, S5mode):
        return self._write111(address, self.Cmd.SETPINFUNCTIONS, S3mode,
                              S4mode, S5mode)
    ...
    def WriteEeprom(self, address, ee_address, ee_word):
        retval = self._write111(address, self.Cmd.WRITEEEPROM, ee_address,
                                ee_word >> 8, ee_word & 0xFF)
        if retval == True:
            trys = self._trystimeout
            while 1:
                self._port.flushInput()
    ...
    def Open(self):
        try:
            self._port = serial.Serial(port=self.comport,
                                       baudrate=self.rate,
                                       timeout=1,
