    ...
    if isinstance(default, dict):
        ...
        if type(new_parameter) != type(default):
            ...
            def check2(type_new, type_old):
                return isinstance(new_parameter, type_new) and isinstance(
                    default, type_old)
...
class RoboclawSingle:
    ...
    def SetSerialTimeout(self, value):
        def SetSerialTimeout(timeout):
            ...
            return self.rc._write1(self.address, SETSERTIALTIMEOUT,
                                   int(timeout * 10))

        return self.safe_call(SetSerialTimeout,
                              value,
                              message=f'Could not set communication timeout')
    ...
    def ReadSerialTimeout(self):
        ...
        return self.safe_call(ReadSerialTimeout,
                              self.address,
                              message=f'Could not read communication timeout')

    def GetConfig(self):
        config_value = self.safe_call(self.rc.GetConfig,
                                      self.address,
                                      message=f'Could not read configs')
        ...
        if config_value[1] != self.config_value[1]:
            ...
            def get(key):
                ...
                items = [(int(key), val) for key, val in items
                         if not val or config_value & val]
    ...
    def SetConfig(self, **kwargs):
        ...
        if self.config_value[0]:
            ...
            if self.config_value[1] == config_value:
                return True
            if self.safe_call(self.rc.SetConfig,
                              self.address,
                              config_value,
                              message=f'Could not set configs'):
    ...
    def ReadCurrents(self, *args, **kwargs):
        return self.safe_call(self.rc.ReadCurrents,
                              self.address,
                              message=f'Could not read motors currents')

    def ReadPWMs(self, *args, **kwargs):
        pwms = self.safe_call(self.rc.ReadPWMs,
                              self.address,
                              message=f'Could not read motors duty')
        if pwms[0]:
            (success, m1, m2) = pwms
    ...
    def ReadEncoderModes(self, motor=None):
        encoder_mode_value = self.safe_call(
            self.rc.ReadEncoderModes,
            self.address,
    ...
    def ReadPositionPID(self, *args, motor, **kwargs):
        return self.motor_call(
            ...
            *args,
            message=
            f'Could not read motor {motor} default position PID settings',
            **kwargs)
    ...
    def ReadVelocityPID(self, *args, motor, **kwargs):
        return self.motor_call(
            ...
            *args,
            message=
            f'Could not read motor {motor} default velocity PID settings',
            **kwargs)

    def ResetEncoders(self, *args, **kwargs):
        return self.safe_call(self.rc.ResetEncoders,
                              self.address,
                              message=f'Could not reset encoders')
    ...
    def SetPositionPID(self, *args, motor, **kwargs):
        def SetPositionPID(address, kp, ki, kd, kimax, deadzone, min, max,
                           motor):
            if motor == 1:
                CMD = self.rc.Cmd.SETM1POSPID
            else:
                CMD = self.rc.Cmd.SETM2POSPID
            return self._write4444444(address, CMD, int(kd * 1024),
                                      int(kp * 1024), int(ki * 1024), kimax,
                                      deadzone, min, max)
    ...
    def SetVelocityPID(self, *args, motor, **kwargs):
        def SetVelocityPID(address, p, i, d, qpps, motor):
            ...
            return self._write4444(address, self.Cmd.SETM1PID, int(d * 65536),
                                   int(p * 65536), int(i * 65536), qpps)
    ...
    def ReadNVM(self, *args, **kwargs):
        return self.safe_call(
            self.rc.ReadNVM,
            self.address,
            message=f'Could not load roboclaw non-volatile memory')
...
class Node:
    def __init__(self, name):
        ...
        device = loadParameter('~device', "/dev/ttyACM0")
        baudrate = loadParameter('~baudrate', 115200)
        self.rate = loadParameter('~loop_rate', 10)

        self.rc = roboclaw.Roboclaw(device, baudrate)
        self.driver = Driver(self.rc, rate=self.rate)
...
class Motor:
    ...
    def __init__(self, roboclaw, index=1, namespace="~", rate=10):
        ...
        self.joint_state = JointState(velocity=[0],
                                      position=[0],
                                      acceleration=[0],
                                      effort=[0],
                                      duty=[0])
        ...
        # Subscribe to topics
        rospy.Subscriber(f'{self.namespace}set_joint_state', JointState,
                         self.setJointState)
        rospy.Subscriber(f'{self.namespace}add_encoder_offset', JointState,
                         self.addEncoderOffset)

        # Define topics publishers
        self.publisher = rospy.Publisher(f'{self.namespace}joint_state',
                                         JointState,
                                         queue_size=1)
    ...
    def setVelocity(self, velocity, acceleration=None):
        if acceleration is None:
            ...
            self.rc.SpeedAccel(abs(acceleration),
                               self.rad2ticksM1(velocity),
                               motor=self.index)

    def limitEffort(self, effort=None):
        # Don't limit effort If it's not specified
        current = self.max_current if effort is None else min(
            self.max_current,
            abs(effort) / self.torque_constant)
    ...
    def updateDistance(self):
        ...
        if encoder_distance[0]:
            self.joint_state.position = [
                self.ticks2rad(encoder_distance[1] + self.encoder_offset)
            ]
    ...
    def updateAcceleration(self):
        dv = self.previous_joint_state.velocity[0] - self.joint_state.velocity[
            0]
        dt = (self.previous_joint_state.header.stamp -
              self.joint_state.header.stamp).to_sec()
        if dt:
            current_accel = dv / dt
            previous_accel = self.joint_state.acceleration[0]
            alpha = 1 / self.rate
            self.joint_state.acceleration = [
                previous_accel + (1 - alpha) * (current_accel - previous_accel)
            ]
    ...
    def updateEffort(self):
        ...
        if encoder_current[0]:
            ...
            self.joint_state.effort = [
                encoder_current[self.index] * self.torque_constant / 100
            ]

    def getPositionPID(self):
        (P, I, D, MaxI, Deadzone, MinPos,
         MaxPos) = self.rc.ReadPositionPID(motor=self.index)[1:]
    ...
    def updateParameters(self):
        ...
        if load('absolute_encoder') or load('reverse_encoder') or load(
                'reverse_motor'):
...
class Driver:
    def __init__(self, roboclaw, namespace="~", rate=10):
        ...
        try:
            ...
            return
        # Load address
        self.address = loadParameter(f'{self.namespace}address', 128)
    ...
    def updateParameters(self):
        ...
        if not hasattr(self, 'motors'):
            ...
            for motor in self.motors:
                self.motor[str(motor)] = Motor(self.rc,
                                               motor,
                                               self.namespace,
                                               rate=self.rate)
        if load('motors'):
        ...
        if load('multiunit_mode') or load('relay_mode') or load(
                'swap_buttons') or load('swap_encoders'):
